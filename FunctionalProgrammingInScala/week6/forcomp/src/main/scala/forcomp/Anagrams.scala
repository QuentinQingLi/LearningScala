...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    ...
    //(w.toLowerCase groupBy (c=>c) map {case(k,v) => (k,v.length)} toList).sorted
  }
  ...
  def wordAnagrams(word: Word): List[Word] = {
    ...
    /*for{
      w <- dictionary
      if wordOccurrences(w) == wordOccurrences(word)
    } yield w
    */
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    ...
    def allOccuForSinglePair(pair: (Char, Int)): List[Occurrences] = {
      (for {i <- 1 to pair._2 by 1} yield List((pair._1, i))).toList
    }

    def combineTwoOccuLists(left: List[Occurrences], right: List[Occurrences]): List[Occurrences] = {
      for { x <- left; y <- right } yield (x ++ y).sorted
    }

    occurrences match {
      case Nil => List(List())
      case occu :: Nil => combinations(Nil) ::: allOccuForSinglePair(occu)
      case occu :: ys =>
        val rest = combinations(ys)
        val x = List(List()) ++ ((1 to occu._2) map (i => List((occu._1, i)))).toList
        combineTwoOccuLists(x, rest)
    }
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    y.foldLeft(x) (
      (x, yy) => {
        (x.filter(p=>p._1!=yy._1)) ++
          (for{xx<-x; if(xx._1==yy._1 && xx._2>yy._2)} yield (xx._1,xx._2-yy._2))
      }
    )
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    ...
    */
    ...
    def fillSentence(targetOccu:Occurrences): List[Sentence] = {
      if(targetOccu.isEmpty) {
        List(Nil)
      }
      else {
        for {
    ...
    fillSentence(sentenceOccurrences(sentence))
  }
}
